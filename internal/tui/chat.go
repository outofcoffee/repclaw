...
func newChatModel(b backend.Backend, sessionKey, agentID, agentName, modelID string, prefs config.Preferences, hideInput bool, connName string) chatModel {
	...
	// produce a visible block, but on some native-platform terminals
	// index 7 maps to a dimmer shade and the reverse-swapped block
	// collapses to near-invisible against the dim placeholder — the
	// leading "T" of "Type a message..." disappears under the cursor.
	// Pinning to ANSI 15 (bright white) removes the dependency on
	// palette luminance choices: every reasonable terminal renders
	// index 15 as bright white, so the swap puts white in the cell's
	// bg and the terminal's default-bg colour in the fg — a
	// guaranteed-visible block. SetStyles propagates through the
	// textarea's `updateVirtualCursorStyle` so `virtualCursor.Style`
	// picks up the new Foreground.
	taStyles := ta.Styles()
